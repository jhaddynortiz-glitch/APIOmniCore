...
export class UsersService {
  ...
  async addUserToOrganization(email: string, role: string, organizationId: string): Promise<any> {
    ...
    if (!user) {
      throw new NotFoundException('Este usuario no está registrado en la plataforma. Debe registrarse primero.');
    }

    // 1.5. Verificar que el usuario no sea SUPER_ADMIN
    if (user.globalRole === 'SUPER_ADMIN') {
      throw new ConflictException('No se puede invitar a un Super Admin a una organización, ya tienen acceso total.');
    }
    ...
    if (existing) {
      throw new ConflictException('Este usuario ya pertenece a esta organización.');
    }
  ...
  async updateOrgMember(userId: string, organizationId: string, data: { role?: string, status?: string }) {
